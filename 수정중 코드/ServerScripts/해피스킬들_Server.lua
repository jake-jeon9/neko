...
Server.sayCallback = function(u,text,sayType)
    print(sayType == 0 .."트페"..sayType)
    ...
    elseif string.match(text,"/가족 ") then
        return false
    elseif (u.unit.GetStringVar(textColor) ~= nil or u.unit.GetStringVar(textColor)~="") and sayType == 0 then
        print(sayType.." < 세이타입")
        Server.onSay.add(hasTextColor(u.unit,text))
        return false
    else
        return true
    end
 ...
 function hasTextColor(unit,text)
    if unit.GetStringVar(textColor) ~= nil or unit.GetStringVar(textColor) ~="" then
        unit.Say("<color="..unit.GetStringVar(textColor)..">"..text.."</color>" )
        return 
    end
 end
-------------------------------onSay-------------------------------
