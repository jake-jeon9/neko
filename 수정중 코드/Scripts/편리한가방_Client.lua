...
function sooni_mon(Q, W, E, R, T, Eq)
	...
	Full_panel.color = Color(0, 48, 8, 255)
	
	local Close_button = Button('✖', Rect(Full_panel.width-36, 0, 36, 36))
	Close_button.color = Color(0, 48, 8, 255)
	...
	Assi_panel.color = Color(0, 48, 8, 255)
	...
	local key_button = {}
	for i=1, 7 do
		key_button[i] = Button('', Rect(58*(i-1)+5, Sero-45, 52.5, 40))
	...
	key_button[1].onClick.Add(function()
		...
		key_button[2] = Button('', Rect(58*(2-1)+5, Sero-45, 52.5, 40))
		Full_panel.AddChild(key_button[2])
		key_button[2].color = classic_COLOR
		key_button[2].textAlign = 4
		key_button[2].textSize = 16
	...
	--[[
	local Input_text = nil
	key_button[2].placeholder = '숫자입력'
	key_button[2].onClick.Add(function()
		Input_text = key_button[2].text
		if Input_text then return end
		key_button[2] = InputField(Rect(0, 0, key_button[3].width, key_button[3].height))
		key_button[2].textSize = 14
		key_button[2].textAlign = 4
		key_button[2].color = Color(255, 255, 255, 255)
		key_button[2].placeholder = "<size=10><color=#FFFFFF>숫자입력\n하세요...</color></size>"
	end)

	]]
	
	key_button[3].text = '판매'
...
--사용하기 유아이 시작
	...
	key_button[5].onClick.Add(function()
		...
		--사용 가능한 아이템이 아닐경우
		local itemName = ""
		local isPossibleToUse = getAvailbleItem(real_dataID)

		if isPossibleToUse then 
			itemName = getitemName(real_dataID)
		else
			Client.ShowAlert('현재 사용 가능한 아이템이 아닙니다.')
			return  
		end
		if Input_text ~= nil then
			txt = string.sub(Input_text.text, string.find(Input_text.text, '%d+')) --문자열을 제외하고 숫자만 가져옴
		...
		Client.ShowYesNoAlert(string.format( "%s 아이템을 %d개 사용 하시겠습니까?",itemName,txt ), function(a)
			if a == 1 then
				local real_item = {}
				local save_count = {}
				for i, p in pairs(Q) do
				...
				if #real_item >= 1 then
					Client.FireEvent("using_item", Utility.JSONSerialize(real_item), tonumber(txt),tonumber(real_dataID))
				else
					Client.ShowAlert('선택하신 아이템이 없습니다.')
				end
			end
	...
	key_button[6].onClick.Add(function()
		...
		Client.ShowYesNoAlert("선택한 "..itemName.."을 모두 사용하시곘습니까?\n<color=#FE2E2E>개수와 상관없이 모두 사용됩니다.</color>", function(a)
			if a == 1 then
				local real_item = {}
				for i, p in pairs(Q) do
					if Item_button[i].color.r == blue_COLOR.r and Item_button[i].visible then
				...
				if #real_item >= 1 then
					--print("클라 호출")
					Client.FireEvent("using_itemALL", Utility.JSONSerialize(real_item), tonumber(real_dataID))
				else
					Client.ShowAlert('선택하신 아이템이 없습니다.')
				end
			end
	...
	key_button[7].text = '모두제작'
	key_button[7].onClick.Add(function()
		local real_item = {}
		local real_dataID = 0
		local itemName = ""
		for i, p in pairs(Q) do
		...
		--사용 가능한 아이템이 아닐경우
		local isPossibleToUse = IsChangeItem(real_dataID)

		if isPossibleToUse then 
			itemName = getitemName(real_dataID)
		else
			Client.ShowAlert('현재 제작 가능한 아이템이 아닙니다.')
			return  
		end
		...
		--아이템 교환 비
		if real_dataID == 12 then -- 교환 할 재료 아이템
			count = 10
			targetName = '전설의물약'
			targetNum = 5-- 교환 받을 아이템
		elseif real_dataID == 340 then
			count = 100
			targetName = '전밥'
			targetNum = 344
		elseif real_dataID == 346 then
			count = 30
			targetName = '막대사탕'
			targetNum = 35
		end
		
		Client.ShowYesNoAlert(itemName.."를 "..count.."당"..targetName.." 1개로 교환하시겠습니까?\n<color=#FE2E2E>개수와 상관없이 모두 사용됩니다.</color>", function(a)
			if a == 1 then
				local real_item = {}
				for i, p in pairs(Q) do
					if Item_button[i].color.r == blue_COLOR.r and Item_button[i].visible then
				...
				if #real_item >= 1 then
					--print("클라 호출")
					Client.FireEvent("change_itemA", Utility.JSONSerialize(real_item), tonumber(real_dataID),tonumber(targetNum),tonumber(count),targetName)
				else
					Client.ShowAlert('선택하신 아이템이 없습니다.')
				end
			end
...
function getAvailbleItem(real_dataID) -- 해당 번호가 사용 가능한지?
		local list = {5,4,6,11,12} -- 아이템 목록 ,교환받을아이템
	local values = false

	for key,value in pairs(list) do
		if value == real_dataID then
			values = true
			return values
		end
	end
...
function IsChangeItem(real_dataID) -- 해당 번호가 사용 가능한지?
	local list = {12} -- 아이템 목록 , 교환 할 아이템
local values = false
...
function getitemName(real_dataID)
	--print("진입")
	local list = {"전설의밥","전설의밥2","전설의밥3","전설의물약","전설의물약(출석)"}
	local name = ""
	--print("realdata?"..real_dataID)
	if real_dataID == 4 then
		name = list[1]
	elseif real_dataID == 11 then
		name = list[2]
	elseif real_dataID == 12 then
		name = list[3]
	elseif real_dataID == 5 then
		name = list[4]
	elseif real_dataID == 6 then
		name = list[5]
	end
